...
1. GET `/admin/editors` — список пользователей с ролью editor
2. PUT `/admin/users/{user_id}/role` — назначить/снять роль editor
3. Editor видит и редактирует только маршруты с `created_by_user_id = current_user.id`
4. Editor может публиковать только свои маршруты

**US-033:** Как редактор маршрутов, я хочу иметь доступ к draft-версии маршрута, которая хранится в виде JSON-объекта в таблице routes, чтобы редактировать маршрут до публикации.

#### Acceptance Criteria (US-033):
1. Draft маршрута хранится в поле `routes.draft_geojson: JSONB`
2. GET `/routes/{route_id}/draft` — получить draft для редактирования
3. PATCH `/routes/{route_id}/draft` — обновить draft
4. При публикации draft копируется в route_version и распарсивается по чекпоинтам
...
1. Upload: паттерн PUT `/audio/{route_id}/{lang}/poi_{seq_no}.mp3`, автоматический matching, manual fallback для unmatched
3. Storage path: GET `/audio/{route_id}/{lang}/poi_{seq_no}.mp3`

**US-041:** Как редактор контента, я хочу указывать проектируемые языки маршрута и отмечать языки готовые к публикации, чтобы пользователи видели только полностью готовые языковые версии.
