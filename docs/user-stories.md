...
9. **✅ RESOLVED: Grandfathering "навсегда":**
   - Буквально навсегда, без TTL и ограничений
   - Права привязаны к `account_id`, не к email
   - Хранить `started_at` timestamp + `price_at_start` для аудита
   - Защита от abuse не требуется (trust users & editors)
